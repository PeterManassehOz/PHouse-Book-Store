 ...
 const schema = yup.object().shape({
   ...
   return (
     <div className={`min-h-screen flex items-center justify-center ${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-black"}`}>
       ...
       >
         <h2 className={`text-2xl font-semibold text-center mb-6 ${darkMode ? "text-white" : "text-black" }`}>Chief Admin Sign Up</h2>
         ...
         <div className={`text-center mt-4 ${darkMode ? "text-white" : "text-gray-600" }`}>
           Don't have an account? <Link to="/admin-signup" className={`${darkMode ? "text-amber-500" : "text-amber-700" }`}>Admin Sign up</Link>
         </div>
       </form>
     </div>
   );
