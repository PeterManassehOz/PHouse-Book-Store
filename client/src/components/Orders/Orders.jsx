...
const OrderCard = ({ order }) => {
  ...
  return (
    ...
    >
      <h3 className="text-lg font-semibold mb-2">
        Order ID:{" "}
        <span className={`${darkMode ? "text-white" : "text-blue-600"} font-semibold`}>
          {order._id}
        </span>
      </h3>
      <p>
        Date:{" "}
        <span className={`${darkMode ? "text-white" : "text-gray-600"} font-medium`}>
          {new Date(order.createdAt).toLocaleString()}
        </span>
      </p>
      <p className="mt-1">
        Status:{" "}
        <span className={`inline-block px-2 py-1 rounded text-sm font-semibold ${badgeClass}`}>
          {statusText}
        </span>
      </p>
      ...
      <h4 className={`mt-4 text-lg font-semibold ${darkMode ? "text-white" : "text-black"}`}>
        Products:
      </h4>
      <div className="grid grid-cols-1 gap-3 mt-2">
        {order.items.map((item) => (
          ...
          >
            ...
            <div className="mt-2">
              ...
              <p className="text-sm">Author: {item.author[0]?.name}</p>
             
                 {/* Price per unit */}
                  <p className={`text-sm ${darkMode ? "text-white" : "text-amber-600"}`}>
                  ...
                  {/* Line-item subtotal */}
                   <p className={`text-sm font-medium ${darkMode ? "text-white" : "text-green-600"}`}>
